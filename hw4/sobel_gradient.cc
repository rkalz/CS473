...
cv::Mat x_filter(3, 3, CV_32F, sobel_x_filter_data);
cv::Mat y_filter(3, 3, CV_32F, sobel_y_filter_data);
...
cv::Mat sobel_gradient_magnitude(const cv::Mat& input) {
  ...
  for (int i = 0; i < input.rows; i++) {
    for (int j = 0; j < input.cols; j++) {
      auto gx = single_filter(input, x_filter, i, j);
      auto gy = single_filter(input, y_filter, i, j);
